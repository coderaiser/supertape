...
- showing colored diff when using the [`t.equal()`](#tequalresult-any-expected-any-message) and [`t.deepEqual()`](#tdeepequalresult-any-expected-any-message) assertion operators
- detailed stack traces for `async` functions
- multiple [`test.only`'s](#testonlymessage-fn-t-test--options-testoptions)
