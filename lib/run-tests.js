...
const notOk = (actual, message = 'should be truthy') => {
    return {
        is: !Boolean(actual),
        expected: true,
        actual,
...
const jsonEqual = (expected, actual, message = 'should equal') => {
    const {is, output} = deepEqual(json(a), json(b));
...
const initOperator = ({out}) => (name) => (...a) => {
    const {is, message, expected, actual, output, stack} = operators[name](...a);
