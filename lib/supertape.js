...
function _createEmitter({quiet, formatter}) {
    const tests = [];
    
    const emitter = new EventEmitter();
    
    const {harness, reporter} = createReporter(formatter);
...
const createStream = () => {
    const {formatter} = initedOptions;
    const {harness} = createReporter(formatter);
